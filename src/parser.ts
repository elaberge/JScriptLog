...
import {
  getFinalEnclosure, newBlankEnclosure, newErrorException,
  newSubtermEnclosure, newTermEnclosure
} from "./enclosures";
import {
  newAtomGoal, newAtomGoalFromRuleSet, newVariableGoal
} from "./goals";
import {
  addRule, addRuleSet, RuleSet, newRule, newFunctionRule
} from "./kb";
import {
  isConstant, isListNull, isListPair, isNumber,
  isObjectReference, newAtom, newConsPair, newConsPairsFromTerms,
  newConstant, newListFromTerms, newListNull, newListPair,
  newNumber, newObjectReference, newOrPair,
  newOrPairsFromTerms, newRuleTerm, newVariable
} from "./types";
import {
...
function jslog_query(in_src: any)
{
}

function jslog_consult(in_src: any, kb: any)
{
}
...
function jslog_plog_token(name: any, symbol: any, kb: any)
{var inlist,outlist;
...
function jslog_plog_range_token(name: any, symbol_min: any, symbol_max: any, kb: any)
{var pairs;
...
// plog:token:name*(I,I,[]).
function jslog_plog_zero_or_more_token(name: any, kb: any)
{
 addRuleSet(kb,new RuleSet('plog:token:'+name+'*',3,false));
...
// plog:token:name+(I,O,N) :- plog:token:name(I,S,N1), !, plog:token:name*(S,O,N2), internal:append(N1,N2,N).
function jslog_plog_one_or_more_token(name: any, kb: any)
{
 addRuleSet(kb,new RuleSet('plog:token:'+name+'+',3,false));
...
export function jslog_library_parser(kb: any)
{
...
function parser_makeobject_var_fn(goal: any)
...
function parser_makeobject_number_fn(goal: any)
{var encl = getFinalEnclosure(goal.encl);
 var lhs = getFinalEnclosure(newSubtermEnclosure(encl.enclosure,encl.term.children[0]));
 var rhs = newSubtermEnclosure(encl.enclosure,encl.term.children[1]);
...
function parser_makeobject_constant_fn(goal: any)
{var encl = getFinalEnclosure(goal.encl);
 var lhs = getFinalEnclosure(newSubtermEnclosure(encl.enclosure,encl.term.children[0]));
 var rhs = newSubtermEnclosure(encl.enclosure,encl.term.children[1]);
...
function parser_makeobject_atom_fn(goal: any)
{var encl = getFinalEnclosure(goal.encl);
 var lhs = getFinalEnclosure(newSubtermEnclosure(encl.enclosure,encl.term.children[0]));
...
function parser_makeobject_list_fn(goal: any)
{var encl = getFinalEnclosure(goal.encl);
 var lhs = getFinalEnclosure(newSubtermEnclosure(encl.enclosure,encl.term.children[0]));
 var rhs = newSubtermEnclosure(encl.enclosure,encl.term.children[1]);
 var arr = new Array();
...
function parser_makeobject_nulllist_fn(goal: any)
{var encl = getFinalEnclosure(goal.encl);
...
function parser_objectmake_enclosure_fn(goal: any)
{var encl = getFinalEnclosure(goal.encl);
 var lhs = getFinalEnclosure(newSubtermEnclosure(encl.enclosure,encl.term.children[0]));
 var rhs = newSubtermEnclosure(encl.enclosure,encl.term.children[1]);
